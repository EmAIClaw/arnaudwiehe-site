...
async function collectArticles() {
  if (!(await fileExists(publishedRoot))) {
    if (await fileExists(outputPath)) {
      console.log(`Published content directory not found at ${publishedRoot}; keeping committed ${path.relative(process.cwd(), outputPath)}.`)
      return null
    }

    throw new Error(`Published content directory not found at ${publishedRoot} and ${outputPath} does not exist.`)
  }
